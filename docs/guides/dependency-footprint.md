...
`askllm` supports 17 LLM providers, but no single install needs all of
their SDKs at once. `pip install askllm` installs only `PyYAML` (used by
the built-in default-model catalog) — every provider SDK is an opt-in
extra via `pip install askllm[<provider>]` (see
...
Installs only `askllm` itself plus its single dependency, `PyYAML`. This
is fully functional on its own — `import askllm`, `askllm.ask(...)`,
`askllm.get_provider(...)` all work for constructing/inspecting objects.
Only *calling* a provider requires its extra:

```pycon
>>> import askllm
>>> askllm.ask("hi", provider="anthropic", model="claude-haiku-4-5-20251001")
ImportError: Provider 'anthropic' requires additional dependencies that
are not installed. Install with: pip install askllm[anthropic]
```

## Per-provider package counts (measured sample)

| Extra | Total packages (incl. askllm) | What makes up the difference |
|---|---:|---|
...
completions client (`askllm/providers/openai_compat.py`) and need
...
```bash
python -m venv /tmp/askllm-check-<extra>
/tmp/askllm-check-<extra>/bin/pip install -e ".[<extra>]"
/tmp/askllm-check-<extra>/bin/pip list
```
...
ordering should hold. See `askllm/registry.py` for the lazy-loading
