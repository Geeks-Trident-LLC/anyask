...
askllm integrates 17 LLM providers behind one interface — `ask()`,
...
There is no automatic routing or fallback anywhere in askllm — `provider`
...
implementation (`askllm/providers/openai_compat.py`) and differ only in
base URL, env var, and default model. Each still needs its own extra
installed (e.g. `pip install askllm[groq]`) since credentials and default
...
extending askllm itself, not for calling it:
...
- **Shape A** — OpenAI-compatible chat-completions API (DeepSeek, Groq,
  ...
  (`askllm/providers/openai_compat.py`) and differ only in base URL, env
...
namespace. Since askllm has no auto-routing to begin with, this is never
